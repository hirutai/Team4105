...
public: // メンバ関数
	...
	XMFLOAT3 GetLightPos()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			else if (pointLights[i].IsActive())
			{
				g_pos = pointLights[i].GetLightPos();
			}
			else if (spotLights[i].IsActive())
			{
				g_pos = spotLights[i].GetLightPos();
			}
		}
	...
	XMVECTOR GetLightVec()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			if (dirLights[i].IsActive())
			{
				gvec = dirLights[i].GetLightDir();
			}
			else if (spotLights[i].IsActive())
			{
				gvec = spotLights[i].GetLightDir();
			}
		}
